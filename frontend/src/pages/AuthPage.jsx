...
export default function AuthPage({ mode }) {
  ...
  useEffect(() => {
    ...
    const state = searchParams.get('state'); // 'google' | 'microsoft'
  ...
  const googleSignIn = () => {
    if (!oauthCfg.google?.enabled) { toast.info('Google OAuth not configured'); return; }
    const redirectUri = `${window.location.origin}/auth/callback`;
      ...
      <div className="flex-1 flex flex-col justify-center items-center p-6">
        <div className="w-full max-w-sm animate-fade-in-up">
          ...
          <div className="mt-6 space-y-2">
            <Button variant="outline" className="w-full justify-center" onClick={googleSignIn} data-testid={AUTH.googleBtn} disabled={loading}>
